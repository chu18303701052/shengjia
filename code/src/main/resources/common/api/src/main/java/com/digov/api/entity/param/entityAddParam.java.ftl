 ...
 */
<#-- 定义不需要生成的字段、隐藏的问题-------->
<#assign noCreateList=["id", "flag"] hiddenList=["createUserId", "createTime", "modifyTime"]>
@Setter
@Getter
@ApiModel(value = "${entity}AddParam", description = "参数")
...
<#list table.fields as field>
<#-- -----   存在字段填充设置   ----->
    <#if field.propertyName != "id" && field.propertyName != "flag" && field.propertyName != "createTime" && field.propertyName != "modifyTime">
